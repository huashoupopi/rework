...
def _strip_stream_markers(full_response: str) -> str:
    content_only = re.sub(
...
async def chat_stream(
    ...
    try:
        session_messages, image_context, vision_image_paths = await _assemble_locked_chat(
            db=db,
    ...
    # 锁只在 generator.finally 释放，不能包在路由外层 finally
...
async def _assemble_locked_chat(
    db: AsyncSession,
    current_user: user.User,
    conv: Any,
    owned_task: task_model.Task | None,
    prepared_images: list,
    question: str,
) -> tuple[list[dict[str, str]], dict | None, list[str]]:
    ...
    return session_messages, image_context, vision_image_paths
    ...
    async with AsyncSessionLocal() as bg_session:
        try:
            ...
            if not _stream_stopped:
                ...
                if remaining:
                    full_response += remaining
            content_only = _deduplicate_content(_strip_stream_markers(full_response))
            ...
            if full_response.strip():
                partial = _strip_stream_markers(full_response)
                if partial:
                    meta_cancel: dict = {}
                    if parser.think_content:
